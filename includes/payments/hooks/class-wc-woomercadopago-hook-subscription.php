...
/**
 * Class WC_WooMercadoPago_Hook_Subscription
 */
class WC_WooMercadoPago_Hook_Subscription extends WC_WooMercadoPago_Hook_Abstract {
	...
	public function load_hooks( $is_instance = false ) {
		...
		if ( ! empty( $this->payment->settings['enabled'] ) && 'yes' === $this->payment->settings['enabled'] ) {
			add_action( 'wp_enqueue_scripts', array( $this, 'add_checkout_scripts_subscription' ) );
			add_action( 'woocommerce_after_checkout_form', array( $this, 'add_mp_settings_script_subscription' ) );
			add_action( 'woocommerce_thankyou', array( $this, 'update_mp_settings_script_subscription' ) );
		}
	 ...
	 */
	public function add_checkout_scripts_subscription() {
	...
	/**
	 * Scripts to subscription
	 */
	public function add_mp_settings_script_subscription() {
		parent::add_mp_settings_script();
	}

	/**
	 * Update settings script subscription
	 *
	 * @param string $order_id Order Id.
	 */
	public function update_mp_settings_script_subscription( $order_id ) {
